...
for entry in $(echo $DAILY_QUESTS | jq -r '.[] | @base64'); do
  ...
  for quest in $(echo $QUESTS | jq -r '.[] | @base64'); do
    ...
    echo "starkli deploy --network mainnet --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $CLASS_HASH $CALLDATA"
    QUEST_DEPLOY_OUTPUT=$(starkli deploy --network mainnet --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $CLASS_HASH $CALLDATA 2>&1)
  ...
  echo "starkli invoke --network mainnet --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $ART_PEACE_CONTRACT_ADDRESS add_daily_quests $DAY_IDX ${#DAILY_QUEST_CONTRACTS[@]} ${DAILY_QUEST_CONTRACTS[@]}"
  starkli invoke --network mainnet --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $ART_PEACE_CONTRACT_ADDRESS add_daily_quests $DAY_IDX ${#DAILY_QUEST_CONTRACTS[@]} ${DAILY_QUEST_CONTRACTS[@]}
  DAILY_QUEST_CONTRACTS=( )
done
...
echo "starkli invoke --network mainnet --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $ART_PEACE_CONTRACT_ADDRESS add_main_quests ${#MAIN_QUEST_CONTRACTS[@]} ${MAIN_QUEST_CONTRACTS[@]}"
starkli invoke --network mainnet --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $ART_PEACE_CONTRACT_ADDRESS add_main_quests ${#MAIN_QUEST_CONTRACTS[@]} ${MAIN_QUEST_CONTRACTS[@]}
