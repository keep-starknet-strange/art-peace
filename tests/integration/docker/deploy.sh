...
CALLDATA=$(echo -n $ACCOUNT_ADDRESS $WIDTH $HEIGHT $PLACE_DELAY $COLOR_COUNT $COLORS $VOTABLE_COLOR_COUNT $VOTABLE_COLORS $DAILY_NEW_COLORS_COUNT $END_TIME $DAILY_QUESTS_COUNT $DEVNET_MODE)
...
ART_PEACE_CONTRACT_ADDRESS=$ART_PEACE_CONTRACT_ADDRESS CANVAS_NFT_CONTRACT=$NFT_CONTRACT_ADDRESS USERNAME_STORE_CONTRACT=$USERNAME_STORE_ADDRESS ACCOUNT_FILE=$ACCOUNT_FILE $SCRIPT_DIR/deploy_quests.sh

ART_PEACE_CONTRACT_ADDRESS=$ART_PEACE_CONTRACT_ADDRESS ACCOUNT_FILE=$ACCOUNT_FILE $SCRIPT_DIR/setup_factions.sh
